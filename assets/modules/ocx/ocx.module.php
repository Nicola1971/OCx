...
jQuery(document).ready(function ($) {
  $(".ocx-datagrid").bootgrid({
    ...
    formatters: {
        ...
        {
            return "<textarea rows=\"3\" cols=\"30\">[[OCxProduct? &id=`" + row.product_id + "` &opencartTpl=`opencartTpl` &fetchimages=`1` &store_dir=`assets/images/ocx`]]</textarea>";
        },
            "code2": function(column, row)
        {
            return "<textarea rows=\"3\" cols=\"40\">[[OCxCategory? &cat=`" + row.category_id + "` &opencartTpl=`opencartTpl` &limit=`50` &fetchimages=`1` &orderby=`price` &store_dir=`assets/images/ocx` &orderdir=`DESC` ]]</textarea>";
        },
        ...
        {
            return "<a target=\"_blank\" class=\"btn btn-sm btn-primary command-edit\" data-row-id=\"" + row.id + "\" href=\"'.$shop_url.'/index.php?route=product/product&product_id=" + row.product_id + "\"><span class=\"fa fa-eye\"></span></a> ";
        },
    		 "viewcat": function(column, row)
        {
            return "<a target=\"_blank\" class=\"btn btn-sm btn-primary command-edit\" data-row-id=\"" + row.id + "\" href=\"'.$shop_url.'/index.php?route=product/category&path=" + row.category_id + "\"><span class=\"fa fa-eye\"></span></a> ";
        }
    }
    });
});
